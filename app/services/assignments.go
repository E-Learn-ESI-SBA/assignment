...
func CreateAssignment(ctx context.Context, db *sql.DB, assignment models.Assignment, teacherId int) error {
	_, err := db.Exec("INSERT INTO assignments (title,description,deadline,promo,groups,module,teacher) VALUES ($1,$2,$3,$4,$5,$6,$7)",
	assignment.Title, assignment.Description,assignment.Deadline,assignment.Promo,assignment.Groups,assignment.Module,teacherId)
	if err != nil {
...
func UpdateAssignment(ctx context.Context, db *sql.DB, assignmentId int, editedAssignment models.Assignment) error {
	_, err := db.Exec("UPDATE assignments SET title = $1, description = $2, deadline = $3, promo = $4, groups = $5 WHERE id = $7",
		editedAssignment.Title, editedAssignment.Description, editedAssignment.Deadline, editedAssignment.Promo, editedAssignment.Groups, editedAssignment.Module, assignmentId)
	if err != nil {
